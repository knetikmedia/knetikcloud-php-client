# KnetikCloud\Rule_Engine_VariablesApi
...
$api_instance = new KnetikCloud\Api\Rule_Engine_VariablesApi(new \Http\Adapter\Guzzle6\Client());

try {
    $result = $api_instance->getBREVariableTypes();
    print_r($result);
} catch (Exception $e) {
    echo 'Exception when calling Rule_Engine_VariablesApi->getBREVariableTypes: ', $e->getMessage(), PHP_EOL;
}
?>
```
...
try {
    ...
    echo 'Exception when calling Rule_Engine_VariablesApi->getBREVariableValues: ', $e->getMessage(), PHP_EOL;
}
?>
```

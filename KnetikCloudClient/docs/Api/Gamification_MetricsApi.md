# KnetikCloud\Gamification_MetricsApi
...
$api_instance = new KnetikCloud\Api\Gamification_MetricsApi(new \Http\Adapter\Guzzle6\Client());
$metric = new \KnetikCloud\Model\MetricResource(); // \KnetikCloud\Model\MetricResource | The new metric

try {
    $api_instance->addMetric($metric);
} catch (Exception $e) {
    echo 'Exception when calling Gamification_MetricsApi->addMetric: ', $e->getMessage(), PHP_EOL;
}
?>
```
